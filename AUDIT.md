...
- **Severity**: High
- **Reproduction**: This would manifest if an inter frame contained adjacent `B_PRED` intra macroblocks, because the second macroblock's B_PRED context would use stale `leftBModes` values instead of the first macroblock's right column. However, the current encoder does not naturally produce this scenario: `processInterMacroblock` does not evaluate/select `B_PRED` in its intra fallback (it only considers 16x16 intra modes). To reproduce, you must either force `B_PRED` selection or construct the macroblocks/context manually in a targeted test.
...
- **Issue**: `var debugMB = false` is an unexported package-level mutable variable used for debug logging via `fmt.Printf`. External users cannot toggle it without modifying the package, so this is primarily an internal debugging/tests concern rather than a directly user-triggerable production behavior. However, keeping ad hoc stdout logging in non-test code paths still creates maintenance risk and can leak unstructured output if enabled during development or internal use.
- **Location**: `macroblock.go:5`
- **Severity**: Low
- **Reproduction**: N/A for external consumers; only package-internal code or tests can enable `debugMB = true`.
- **Resolution**: Clarify that `debugMB` is intended only for internal debugging/tests, and either remove the variable and conditional print statements or gate debug logging behind build tags / a logger interface instead of direct `fmt.Printf` in non-test code paths.
...
- [x] **High**: Add first partition size overflow check in `buildFrameTag` — `bitstream.go:754-760`
- [x] **Medium**: Fix `predict4x4HE` to use actual top-left pixel P — `bpred.go:284-316`
...
- [ ] **Medium**: Implement rate control algorithm — `encoder.go`
...
- [ ] **Low**: Implement AltRef frame prediction and management — `refframe.go`
- [ ] **Low**: Add defensive dimension validation to internal functions — `refframe.go:60-71`
- [ ] **Low**: Implement temporal layer support — `encoder.go`
